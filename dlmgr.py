...
class DownloadManager(object):
    ...
    def _refresh_download_speed(self):
        now = time.time()
        last_speed_check = getattr(self, '_last_speed_check', self._start_time - 5)
        if now - last_speed_check >= 5:
            self._download_speed = self._get_increased_size() / (now - last_speed_check)
    ...
    def _try_to_run_next_job(self, force=False) -> Optional[int]:
    ...
    def _handle_request(self, request):
        ...
        if op == OP.ADD_JOB:
            ...
            job_id = self._try_to_run_next_job(force=True)
            conn.sendall((f'Job {job_id} successfully started' if job_id else 'No pending jobs to start.').encode())
        else:
            err(f'Unknown op: {op}')
        if conn:
            ...
            if self._running_jobs:
                ...
                for job_id in exited_jobs:
                    ...
                    if job and job.file_size:
                        job.refresh_downloaded_size()
                        err(f"Job {job_id} '{job.output_file}' " \
                            + ('not completed' if job.downloaded_size < job.file_size else 'completed') \
                            + f', {job.downloaded_size:,}/{job.file_size:,} downloaded.')
